...
void AppendTest()
{
    ...
    numBytes = openFile->Write("Test Append.", 12);
    if (numBytes < 12)
    {
        printf("Perf test: unable to write %s\n", fileName);
        delete openFile;
    ...
    printf("创建测试文件%s,内容:\n", fileName);
    Print(fileName);
    printf("在尾部追加test/big,内容:\n");
    Append("test/big", fileName, false);
    Print(fileName);
    printf("在中间追加test/big,内容:\n");
    Append("test/big", fileName, true);
    Print(fileName);
}
...
void NAppendTest(){
    ...
    numBytes = openFile->Write("Test NAppend.", 13);
    if (numBytes < 13)
    {
        printf("Perf test: unable to write %s\n", fileName);
        delete openFile;
    ...
    printf("创建测试文件%s,内容:\n", fileName);
    Print(fileName);
    printf("在尾部追加AppendTestFile,内容:\n");
    NAppend("AppendTestFile",fileName);
    Print(fileName);
}
...
void PerformanceTest()
{
    ...
    printf("测试读取******************************************\n");
    FileRead();
    printf("测试UNIX到Nachos追加******************************\n");
    AppendTest();
    printf("测试Nachos到Nachos追加****************************\n");
    NAppendTest();
    printf("测试删除******************************************\n");
    ...
    if (!fileSystem->Remove("AppendTestFile"))
    {
        printf("Perf test: unable to remove AppendTestFile\n");
        return;
    }
    if (!fileSystem->Remove("NAppendTestFile"))
    {
        printf("Perf test: unable to remove NAppendTestFile\n");
        return;
    }
    stats->Print();
}
