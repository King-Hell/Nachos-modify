...
//      decoded to identify，指令代码
//	    operation to do，操作
//	    registers to act on，要改变的寄存器
//	    any immediate operand value，立即数
...
// Routines callable by the Nachos kernel
    ...
    int ReadRegister(int num);	// read the contents of a CPU register，读寄存器

    void WriteRegister(int num, int value);
				// store a value into a CPU register，写寄存器


// Routines internal to the machine simulation -- DO NOT call these 

    void OneInstruction(Instruction *instr); 	
    				// Run one instruction of a user program.执行一条指令
    void DelayedLoad(int nextReg, int nextVal);  	
				// Do a pending delayed load (modifying a reg)
    
    bool ReadMem(int addr, int size, int* value);//读内存
    bool WriteMem(int addr, int size, int value);//写内存
    				// Read or write 1, 2, or 4 bytes of virtual 
				// memory (at addr).  Return FALSE if a 
				// correct translation couldn't be found.
    
    ExceptionType Translate(int virtAddr, int* physAddr, int size,bool writing);//地址转换
    ...
    void RaiseException(ExceptionType which, int badVAddr);//唤醒异常
...
// are in terms of these data structures.
	...
	BitMap *freeFrame;//页号位图
	BitMap *threadMap;//线程号位图
